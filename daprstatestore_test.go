...
import (
	...
	"github.com/chelala/daprstatestore"
...
// previously run dapr sidecar, check Readme.md

func TestRediStore(t *testing.T) {
	// DAPR Environment Variables
	t.Setenv("APP_ID", "daprstatestore-test")
	t.Setenv("DAPR_HOST_IP", "localhost")
	t.Setenv("DAPR_HTTP_PORT", "47777")
	t.Setenv("DAPR_GRPC_PORT", "63337")
	...
	{
		// DaprStateStore
		store, err := daprstatestore.NewDaprStateStore(stateStoreName, []byte("secret-key"))
		if err != nil {
			t.Fatal(err.Error())
		}
	...
	{
		store, err := daprstatestore.NewDaprStateStore(stateStoreName, []byte("secret-key"))
		store.SetSerializer(daprstatestore.JSONSerializer{})
		if err != nil {
			t.Fatal(err.Error())
		}
